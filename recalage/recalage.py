...
def moindres_carres(liste_points):
    ...
    mp = MultiPoint([p[0] for p in liste_points])
    hull = mp.convex_hull
...
def compute_recalage(batis, seuil):
    ...
    nb_points_0 = len(liste_points)
    mp = MultiPoint([p[0] for p in liste_points])
    hull_0 = mp.convex_hull
    ...
    parametres, nb_points, mean, res_max, hull = moindres_carres(liste_points)
    ...
    if union_area==0:
        ...
        iou_hull = intersection.area / union_area
    ...
    if parametres is not None:
        for bati in batis_bd_uni:
            ...
            bati.nb_points_0 = nb_points_0
            bati.iou_hull = iou_hull
    return batis_bd_uni
...
def sauvegarde_projection(batis_bd_uni, output):
    ...
    for i, bati in enumerate(batis_bd_uni):
        ...
        nb_points_0.append(bati.nb_points_0)
        mean.append(bati.mean)
        res_max.append(bati.res_max)
        iou_hull.append(bati.iou_hull)

        d = {"id": id, "TX":TX, "TY":TY, "a":a, "b":b, "nb_points":nb_points, "nb_points_0":nb_points_0, "mean":mean, "res_max":res_max,  "geometry": polygones, "iou_hull":iou_hull}
