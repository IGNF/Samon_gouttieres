...
def save_shapefile_from_polygons(polygons, image_filepath, output_shapefile_filepath, properties_list=None):
    ...
    #srs.ImportFromEPSG(coor_system)
    ###################################################
...
def save_shapefile_from_shapely_polygons(polygons, image_filepath, output_shapefile_filepath):
    ...
    ###############################################################################################################################
    #_, _, coor_system = get_coor_in_space(image_filepath)
    #with fiona.open(output_shapefile_filepath, 'w', driver='ESRI Shapefile', schema=schema, crs=fiona.crs.from_epsg(coor_system)) as c:
    with fiona.open(output_shapefile_filepath, 'w', driver='ESRI Shapefile', schema=schema, crs=fiona.crs.from_epsg(2154)) as c: #
