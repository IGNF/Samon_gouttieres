...
def split_img_tiles(height, width, tile_size, overlap):
    ...
    return tiles_windows_list, offsets_w, offsets_h
...
def main(config, backbone, run_dirpath, img_path, out_dirpath):
    ...
    for img in img_path:
        ...
        if os.path.exists(os.path.join(out_dirpath, file_name + ".shp")):
            continue
        with rasterio.open(img) as src:
            height, width = src.height, src.width
            wins, offsets_w, offsets_h = split_img_tiles(height, width, tile_size=tile_size, overlap=overlap)
            inf = []
