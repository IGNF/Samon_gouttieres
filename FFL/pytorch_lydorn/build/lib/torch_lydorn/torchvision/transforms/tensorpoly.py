...
def polygons_to_tensorpoly(polygons_batch):
    ...
    for i, polygons in enumerate(polygons_batch):
        for polygon in polygons:
            if not np.max(np.abs(polygon[0] - polygon[-1])) < 1e-6:
                # Polygon is open
                is_endpoint = np.zeros(polygon.shape[0], dtype=np.bool_)
                is_endpoint[0] = True
                is_endpoint[-1] = True
            else:
                ...
                polygon = polygon[:-1, :]
                is_endpoint = np.zeros(polygon.shape[0], dtype=np.bool_)
            batch = i * np.ones(polygon.shape[0], dtype=np.longlong)
    ...
    slice_start = 0
    poly_slice = np.empty((len(polygon_list), 2), dtype=np.longlong)
