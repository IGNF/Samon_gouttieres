...
def inference_with_patching(config, model, tile_data, emprise_geom):
    ...
    with torch.no_grad():
        ...
        patch_boundingboxes_tmp = image_utils.compute_patch_boundingboxes((height, width),
                                                                    stride=stride,
                                                                    patch_res=config["eval_params"]["patch_size"])
        
        patch_boundingboxes = []
        for patch_bbox in patch_boundingboxes_tmp:
            polygon_patch_bbox = Polygon.from_bounds(patch_bbox[0], -patch_bbox[3], patch_bbox[2], -patch_bbox[1])
            if emprise_geom.intersects(polygon_patch_bbox).any():
                patch_boundingboxes.append(patch_bbox)
