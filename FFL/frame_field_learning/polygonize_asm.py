...
#         _degrees = 2 * np.ones(_coordinates.shape[0], dtype=np.longlong)
#         if not is_closed:
#             _degrees[0] = 1
#             _degrees[-1] = 1
...
def compute_skeletons(seg_batch, config, spatial_gradient, pool=None) -> List[Skeleton]:
    ...
    if config["init_method"] == "marching_squares":
        ...
        corrected_edge_prob_batch = corrected_edge_prob_batch[:, None, :, :].to(torch.float16) # Convet to float for spatial grads
        ...
        if 2 <= seg_batch.shape[1]:
            corrected_edge_prob_batch = torch.clamp(seg_batch[:, 1, :, :].to(config["device"]) + corrected_edge_prob_batch, 0, 1)
        # Save for viz
